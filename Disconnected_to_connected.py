...
P = np.geomspace(0.1/N, 0.8, 300) #list of connectivity probabilities
...
x, y = np.meshgrid(np.log(P), np.log(taus))
plt.subplot(2, 1, 1)
plt.title('Order')
...
for i in I:
    tau = taus[i]
    plt.plot(np.log(P), networks_eta[i, :], label=r'$\tau$=' + str(round(tau, 1)), color=color_dict[i], linewidth=2)
